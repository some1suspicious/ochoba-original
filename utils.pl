...
1 => [qw/ЛОЛИ НЯШКА СПОК
 ПУРУРИН ТИРЕЧ РИНГО ТОРТ АНОН ДВАЧ НЕТ СЕТЬ ПОСТ ФСБ НЯК ПЕКА ПУРУРИН ЛОЛ ТРОЛЛЬ БОРДА КАПЧА ВИТЯ АНИМЕ БЫДЛО ДОБРО ЗЛО/],
...
#4 => [qw/А Б В Г Д Е Ж З И К Л М Н О П Р С Т У Ф Х Ц Ч Ш Ы Э Ю Я/],
#5 => [qw/А Б В Г Д Е Ж З И К Л М Н О П Р С Т У Ф Х Ц Ч Ш Ы Э Ю Я/],
#6 => [qw/А Б В Г Д Е Ж З И К Л М Н О П Р С Т У Ф Х Ц Ч Ш Ы Э Ю Я/],
#7 => [qw/А Б В Г Д Е Ж З И К Л М Н О П Р С Т У Ф Х Ц Ч Ш Ы Э Ю Я/],
#8 => [qw/А Б В Г Д Е Ж З И К Л М Н О П Р С Т У Ф Х Ц Ч Ш Ы Э Ю Я/],
#9 => [qw/А Б В Г Д Е Ж З И К Л М Н О П Р С Т У Ф Х Ц Ч Ш Ы Э Ю Я/]
);

	my ($word,$hash);
	my @s=('a'..'z',0..9);
	$hash.=$s[rand(@s)] for(1...32);
	$word.=${$_}[rand(@{$_})] for(values %words);

	my $img =GD::Image->new(175,30,1);
	$img->fill(1,1,$img->colorAllocate(255,255,255));

	if($^O=~/win/i){$img->stringFT($img->colorAllocate(0,0,0),'C:\nginx\html\DigitalStripCyrillic.ttf',18,0,5,27,$word);}
	else{$img->stringFT($img->colorAllocate(0,0,0),'/home/admin/ochoba/captcha/DigitalStripCyrillic.ttf',18,0,5,27,$word);}
